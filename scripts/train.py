...
def do_train(params: TrainingParams, model_name, weights_folder, resume_filename=None, saved_state_dict=None):
    ...
    val_set = WholeDataset(os.path.join(params.dataset_folder, params.test_database), os.path.join(params.dataset_folder, params.test_query), image_size=params.image_size, dataset=params.dataset)
    ...
    dataloaders = make_dataloaders(params, validation=False) #这里是处理数据集的部分
    ...
    for epoch in tqdm.tqdm(range(start_epoch + 1, params.epochs + 1)):
        ...
        if epoch >100 or epoch % 50 == 0 or epoch==125:
            ...
            if current_val_recall > train_metrics['best_r@1']:
                ...
                torch.save(checkpoint, '/your path/model_best.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',default='/home/long/PycharmProjects/RCPRNet/config/train/snail-n.txt', type=str, required=False, help='Path to configuration file')
    parser.add_argument('--model_config', default='/home/long/PycharmProjects/RCPR/config/model/Test.txt', type=str, required=False, help='Path to the model-specific configuration file')
