...
def make_datasets(params: TrainingParams, validation: bool = True):
    ...
    datasets['train'] = PNVTrainingDataset(params.dataset_folder, params.train_file, params.image_size,params.dataset,
...
def make_collate_fn(dataset: TrainingDataset, quantizer, batch_split_size=None, input_representation="R",):
    ...
    def collate_fn(data_list):
        ...
        if dataset.set_transform is not None:
            clouds = dataset.set_transform(torch.stack(clouds)) #数据增强
        ...
        coords_quant = [quantizer(e)[0] for e in clouds] # 使用0.01米步长对原始笛卡尔坐标(x,y,z)进行离散化（如3.1415→3.14）
        ...
        if batch_split_size is None or batch_split_size == 0:
            ...
            batch = {'coords': coords, 'features': feats, 'batch': clouds, 'image': images}
            ...
            for i in range(0, len(coords), batch_split_size):
                ...
                img_temp = images[i:i + batch_split_size]
                minibatch = {'coords': c, 'features': f, 'batch': batch_temp, 'image': img_temp}
...
def make_dataloaders(params: TrainingParams, validation=True, cluster_nIm=-1):
    ...
    train_collate_fn = make_collate_fn(datasets['train'],  quantizer, params.batch_split_size, params.model_params.input_representation,) #这里是处理数据集的部分
