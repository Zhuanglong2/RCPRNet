...
class PTC_Net(nn.Module):
    ...
    def forward(self, batch):
        ...
        xyz_ts = []
        # -------------------------------Stem stage---------------------------------#
        sparse_tensor = ME.SparseTensor(batch['features'][:, :2], coordinates=batch['coords'])
...
class PTC_Net_L(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.use_sparse_tensor:
            ...
            xyz_tensor = batch['batch'][:, :, :3] #B,4096,3
        ...
        if self.use_top_down:
            # transformer
            for i in range(-1, -(len(self.FP_modules) + 1), -1):
                feature_maps_t[i - 1] = self.FP_modules[i](xyz_ts[i - 1], xyz_ts[i], feature_maps_t[i - 1], feature_maps_t[i])  # up-down
...
class Sample_interpolated(nn.Module):
    ...
    def forward(self, xyz, sparse, feat_t=None):
        ...
        if self.first_sample:
            ...
            center_idx = pointops.furthestsampling(xyz.contiguous(), self.npoint)  # B,npoint
...
class PointNet2FPModule(nn.Module):
    ...
    def forward(self, unknown: torch.Tensor, known: torch.Tensor, unknow_feats: torch.Tensor,
        ...
        if known is not None:
            dist, idx = pointops.nearestneighbor(unknown.contiguous(), known.contiguous())
            dist_recip = 1.0 / (dist + 1e-8)
            norm = torch.sum(dist_recip, dim=2, keepdim=True)
            weight = dist_recip / norm
            interpolated_feats = pointops.interpolation(known_feats, idx, weight)  # upper features interpolate to current layer
        else:
